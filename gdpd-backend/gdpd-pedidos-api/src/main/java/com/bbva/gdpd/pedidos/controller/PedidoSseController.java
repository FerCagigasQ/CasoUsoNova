...
public class PedidoSseController {
    ...
    public void broadcast(PedidoEvent evento) {
        emitters.forEach((key, emitter) -> enviarEvento(emitter, key, evento));
    }

    private void enviarEvento(SseEmitter emitter, String key, PedidoEvent evento) {
        try {
            ...
            log.warn("SSE envio fallido key={}, eliminando emitter", key);
            emitters.remove(key);
        }
    }
}
